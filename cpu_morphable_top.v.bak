...
//     // PIPELINE MODE CONTROLLER V2 (P3/P5/P7)
    
    
//     pipeline_mode_ctrl_v2 mode_ctrl (
//         .clk(clk),
//         .reset(reset),
...
//         .cache_miss(1'b0),  // Not implemented
...
//                          (id_is_jalr ? (jalr_base + branch_imm) & ~32'b1 : branch_target) :
//                          branch_target;
...
module cpu_morphable_top (
    ...
    wire [1:0]  perceptron_predicted_mode;
    wire [2:0]  perceptron_predicted_clk;     // NEW: clock index
    wire [7:0]  perceptron_confidence;
    wire        perceptron_valid;
    ...
    // NEW: Clock training signal — was max clock actually optimal?
    // We determine this by comparing: did the mode at max clock give
    // better effective time than the same mode at lower clock?
    // For simplicity, we always train toward max clock since our
    // Python simulation showed max clock is always optimal.
    wire        actual_best_max_clk;
    assign actual_best_max_clk = 1'b1;  // Max clock is always best (from simulation)
    
    // Clock output for top module
